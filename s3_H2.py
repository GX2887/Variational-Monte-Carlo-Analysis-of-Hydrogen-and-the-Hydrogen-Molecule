...
def energy_and_grad_analytic(theta, theta_ref, R_ref, Ns, q1, q2,
                             delta=0.5, burn_in=200, h=2e-3,
                             seed=None,
                             eps = 1e-3):
    """
    Compute mean local energy E(θ) and its gradient ∇E(θ) analytically
    using ∂ lnψ / ∂θ.
    ...
    # Sample configurations from |ψ|^2 at this θ
    R_samples = rs.metropolis_sample_H2(Ns, theta, q1, q2,
                                     delta=delta, burn_in=burn_in)

    # Local energies
    Els = local_energy_H2_batch(R_samples, theta, q1, q2, h=h, eps=eps)
    E_mean = np.average(Els)
    E_mean_std = np.std(Els)
    # E_mean, E_mean_std = estimate_mean_local_energy_reweighted(theta, theta_ref, R_ref,
    #                                       q1, q2,
    #                                       h=2e-3,
    #                                       eps=1e-3,
    #                                       seed = 123)

    # O_k = ∂ lnψ / ∂θ_k for each sample
    Ns = R_samples.shape[0]
    O = np.zeros((Ns, 3))

    for i in range(Ns):
        O[i, :] = dlogpsi_dtheta(R_samples[i], theta, q1, q2)

    # Gradient estimator: 2 < (E_L - E) O >
    diff = (Els - E_mean)[:, None]          # shape (Ns, 1)
    grad = 2.0 * np.mean(diff * O, axis=0)  # shape (3,)
    G_max = np.array([2,2,2])
    if np.linalg.norm(grad) > np.linalg.norm(G_max):
        grad = G_max
    return E_mean, grad, E_mean_std

def energy_and_grad_reweighted(theta, theta_ref, R_ref, q1, q2,
                               h=2e-3, eps=1e-3):
    """
    Reweighted estimate of E(θ) and ∇E(θ) using a fixed reference
    sample R_ref ~ ρ(R; θ_ref).

    This gives smooth E and gradient as functions of θ,
    provided θ is not too far from θ_ref.
    """
    R_ref = np.asarray(R_ref, dtype=float)
    Ns = R_ref.shape[0]

    # --- 1. Local energies at *target* θ, on same configs R_ref ---
    Els_theta = local_energy_H2_batch(R_ref, theta, q1, q2, h=h, eps=eps)

    # --- 2. Importance weights w ∝ ρ_θ / ρ_θref ---
    rho_theta     = rs.rho_unnormalised_H2_batch(R_ref, theta,     q1, q2, eps=eps)
    rho_theta_ref = rs.rho_unnormalised_H2_batch(R_ref, theta_ref, q1, q2, eps=eps)

    tiny = 1e-16
    w_raw = rho_theta / (rho_theta_ref + tiny)
    w_sum = np.sum(w_raw)

    if w_sum == 0.0:
        # pathological case; fall back to equal weights
        weights = np.ones_like(w_raw) / Ns
    else:
        weights = w_raw / w_sum

    # --- 3. Reweighted mean energy ---
    E_mean = np.sum(weights * Els_theta)

    # --- 4. Log-derivatives O_k = ∂ ln ψ / ∂ θ_k at same R_ref ---
    O = np.zeros((Ns, 3))
    for i in range(Ns):
        O[i, :] = dlogpsi_dtheta(R_ref[i], theta, q1, q2)

    # --- 5. Reweighted gradient: 2 ⟨ (E_L - E) O ⟩_θ ---
    diff = (Els_theta - E_mean)[:, None]          # shape (Ns, 1)
    grad = 2.0 * np.sum(weights[:, None] * diff * O, axis=0)
...
def gradient_search_min_energy_analytic(theta_init, q1, q2,
    ...
    for step in range(n_steps):
        ...
        E, grad, E_std = energy_and_grad_analytic(theta, theta_ref, R_ref, Ns, q1, q2,
                             delta=0.5, burn_in=200, h=2e-3,
                             seed=seed,
                             eps = 1e-3)
        # if step > 200:
...
def El_theta2_scan(theta, q1, q2, Ns=int(1e4), seed=123, step = 101, l= -3.0 , r = 0):
    """
...
# get_project_dir()
theta_mini, history = gradient_search_min_energy_analytic(theta, q1, q2, Ns=int(1e5), n_steps=5, lr=0.01)
t1 = time.perf_counter()
print(f"\nTotal runtime: {t1 - t0:.4f} seconds\n")

# # history = load_history('D:/OneDrive - Imperial College London/Y3 Computing/project/S3_H2/history_gradient_search/history_2025-11-23_06-06-03.pkl')
...
# El_theta2_scan(theta, q1, q2, step = 20 , l = -0.2, r=0.8)
# El_theta3_scan(theta, q1, q2, step = 50, l = 0, r = 1)
